...
!-- Deprecate OMP_NUM_THREADS() and remove overloads as they were not used
!   Use OMP_MAX_THREADS instead.
!-- Initialize OMP_ABSMAX_THREADS within OML_INIT and abort if OML_INIT was forgotten
...
PUBLIC OML_WAIT_EVENT, OML_SET_EVENT, OML_INCR_EVENT, &
   ...
   &   OML_INIT, OML_GET_NUM_THREADS, OML_ABSMAX_THREADS
...
INTEGER(KIND=JPIM) :: N_OML_ABSMAX_THREADS = -1

CONTAINS

SUBROUTINE OML_INIT()
!$ INTEGER(KIND=JPIM) OMP_GET_MAX_THREADS
IF (N_OML_ABSMAX_THREADS == -1) THEN
  N_OML_ABSMAX_THREADS = 1
!$ N_OML_ABSMAX_THREADS = OMP_GET_MAX_THREADS()
ENDIF
...
FUNCTION OML_ABSMAX_THREADS()
INTEGER(KIND=JPIM) :: OML_ABSMAX_THREADS
IF( N_OML_ABSMAX_THREADS == -1 ) THEN
    CALL ABOR1('Please CALL OML_INIT() before accessing OML_ABSMAX_THREADS')
ENDIF
OML_ABSMAX_THREADS = N_OML_ABSMAX_THREADS
END FUNCTION OML_ABSMAX_THREADS
...
FUNCTION OML_NUM_THREADS()
USE EC_LUN, ONLY : NULOUT
INTEGER(KIND=JPIM) :: OML_NUM_THREADS
WRITE(NULOUT,'(A)') "DEPRECATION WARNING: Use OML_MAX_THREADS instead of OML_NUM_THREADS"
OML_NUM_THREADS = OML_MAX_THREADS()
END FUNCTION OML_NUM_THREADS
