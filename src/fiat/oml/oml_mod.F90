...
!**FV/04-Mar-2026
!-- Remove mention of IFS routine in OML_INCR_COUNTER and add optional KERR argument for external error handling.
...
ABSTRACT INTERFACE
SUBROUTINE OML_PARALLEL_FUNCTION (ARGS) BIND(C)
...
SUBROUTINE OML_INCR_COUNTER(K,KMAX,KERR)
INTEGER(KIND=JPIM),INTENT(INOUT) :: K
INTEGER(KIND=JPIM),INTENT(IN) :: KMAX
INTEGER(KIND=JPIM),OPTIONAL,INTENT(OUT) :: KERR
...
IF (K > KMAX)  THEN
  ...
  IF (.NOT. PRESENT(KERR)) CALL ABOR1('OML_INCR_COUNTER: PLEASE INCREASE LOCKS ARRAY DIMENSION.')
ENDIF
...
FUNCTION OML_GET_THREAD_NUM_BINDC() BIND(C,NAME="oml_get_thread_num") RESULT(KMYTID)
   USE, INTRINSIC :: ISO_C_BINDING, ONLY : C_INT
   IMPLICIT NONE
   INTEGER(KIND=C_INT) :: KMYTID
   KMYTID = OML_MY_THREAD() - 1
END FUNCTION
