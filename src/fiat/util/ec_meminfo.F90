...
INTEGER(KIND=JPIM) :: II,I,J,K,MYPROC,NPROC,ERROR,NODENUM,JID
...
CHARACTER(LEN=20)  :: NODENAME,LASTNODE
...
#if defined(MPL_F77_DEPRECATED) || defined(MPL_F90_MODULE)
...
ELSE
   ...
   IPFXLEN = 0
   ZUM = 'sum'
   CALL MPI_COMM_RANK(ICOMM,MYPROC,ERROR)
   CALL CHECK_ERROR("from MPI_COMM_RANK",__FILE__,__LINE__)

   CALL MPI_COMM_SIZE(ICOMM,NPROC,ERROR)
   ...
   IF (MYPROC == 0) THEN
      ...
      DO I=0,NPROC-1
         ...
         IF (I > 0) THEN ! Receive in the MPI-rank order of KCOMM (i.e. may not be the same as MPI_COMM_WORLD -order)
            CALL MPI_RECV(LASTNODE,LEN(LASTNODE),MPI_BYTE,I,ITAG,ICOMM,IRECV_STATUS,ERROR)
            CALL CHECK_ERROR("from MPI_RECV(LASTNODE)",__FILE__,__LINE__)
            CALL MPI_RECV(IORANK,1,MPI_INTEGER4,I,ITAG+1,ICOMM,IRECV_STATUS,ERROR)
            CALL CHECK_ERROR("from MPI_RECV(IORANK)",__FILE__,__LINE__)
            CALL MPI_RECV(K,1,MPI_INTEGER4,I,ITAG+2,ICOMM,IRECV_STATUS,ERROR)
            CALL CHECK_ERROR("from MPI_RECV(RANK_WORLD)",__FILE__,__LINE__)
            CALL MPI_RECV(IPID,1,MPI_INTEGER4,I,ITAG+3,ICOMM,IRECV_STATUS,ERROR)
            CALL CHECK_ERROR("from MPI_RECV(GETPID)",__FILE__,__LINE__)
            CALL MPI_RECV(NUMTH,1,MPI_INTEGER4,I,ITAG+4,ICOMM,IRECV_STATUS,ERROR)
            CALL CHECK_ERROR("from MPI_RECV(NUMTH)",__FILE__,__LINE__)
            CALL MPI_RECV(CLSTR,LEN(CLSTR),MPI_BYTE,I,ITAG+5,ICOMM,IRECV_STATUS,ERROR)
         ...
         IF (I > 0) THEN ! Receive in MPI-rank order
            CALL MPI_RECV(RN(I)%COREIDS,NUMTH,MPI_INTEGER4,I,ITAG+6,ICOMM,IRECV_STATUS,ERROR)
      ...
      DO I=1,NPROC-1
         CALL MPI_SEND(RN(I)%NODEMASTER,1,MPI_INTEGER4,I,ITAG+7,ICOMM,ERROR)
         CALL CHECK_ERROR("from MPI_SEND(IAM_NODEMASTER)",__FILE__,__LINE__)
      ENDDO
   ELSE
      CALL MPI_SEND(NODENAME,LEN(NODENAME),MPI_BYTE,0,ITAG,ICOMM,ERROR)
      CALL CHECK_ERROR("from MPI_SEND(NODENAME)",__FILE__,__LINE__)
      CALL MPI_SEND(IORANK,1,MPI_INTEGER4,0,ITAG+1,ICOMM,ERROR)
      CALL CHECK_ERROR("from MPI_SEND(IORANK)",__FILE__,__LINE__)
      CALL MPI_COMM_RANK(MPI_COMM_WORLD,K,ERROR)
      CALL MPI_SEND(K,1,MPI_INTEGER4,0,ITAG+2,ICOMM,ERROR)
      CALL CHECK_ERROR("from MPI_SEND(RANK_WORLD)",__FILE__,__LINE__)
      IPID = EC_GETPID()
      CALL MPI_SEND(IPID,1,MPI_INTEGER4,0,ITAG+3,ICOMM,ERROR)
      CALL CHECK_ERROR("from MPI_SEND(GETPID)",__FILE__,__LINE__)
      CALL MPI_SEND(MAXTH,1,MPI_INTEGER4,0,ITAG+4,ICOMM,ERROR)
      CALL CHECK_ERROR("from MPI_SEND(MAXTH)",__FILE__,__LINE__)
      CLSTR = CDSTRING
      CALL MPI_SEND(CLSTR,LEN(CLSTR),MPI_BYTE,0,ITAG+5,ICOMM,ERROR)
      CALL CHECK_ERROR("from MPI_SEND(CLSTR)",__FILE__,__LINE__)
      CALL MPI_SEND(COREIDS,MAXTH,MPI_INTEGER4,0,ITAG+6,ICOMM,ERROR)
      CALL CHECK_ERROR("from MPI_SEND(COREIDS)",__FILE__,__LINE__)
      CALL MPI_RECV(IAM_NODEMASTER,1,MPI_INTEGER4,0,ITAG+7,ICOMM,IRECV_STATUS,ERROR)
   ...
   DO NODENUM=1,NN
         ...
         IF (.NOT.DONE(II)) THEN
            ...
            IF (RN(J)%NODENUM == NODENUM) THEN
               ...
               ELSE
                  NRECV = 4
               ENDIF
               CALL MPI_RECV(RECVBUF,NRECV,MPI_INTEGER8,I,ITAG+8,ICOMM,IRECV_STATUS,ERROR)
      ...
      IF (KU == -1) THEN
         ...
         IF (NODENUM == NN) THEN
            ...
            CALL PRT_TOTAL_ENERGIES(KCALL, 0)
            CALL PRT_TOTAL_ENERGIES(KCALL, KULOUT)
            IF (KCALL == 1) THEN
               CALL DATE_AND_TIME(CLDATEOD,CLTIMEOD,CLZONEOD,IVALUES)
               READ(CLDATEOD(5:6),'(I2)') IMON
               CALL PRT_DETAIL(0)
...
ELSE
    ...
    ELSE
       NSEND = 4
    ENDIF
    CALL MPI_SEND(SENDBUF,NSEND,MPI_INTEGER8,0,ITAG+8,ICOMM,ERROR)
...
SUBROUTINE PRT_TOTAL_ENERGIES(KCALL, KUN)
IMPLICIT NONE
...
SUBROUTINE CONDBARR(KBARR)
INTEGER(KIND=JPIM), INTENT(IN) :: KBARR
IF (NPROC > 1 .and. KBARR /= 0) THEN
   CALL MPI_BARRIER(ICOMM,ERROR)
ELSE
   ERROR = 0
ENDIF
