...
CHARACTER(LEN=128) :: CLCSVNAME
CHARACTER(LEN=32) :: CLCSVCOLS(15)
...
IF (LSTATS .AND. LCSV_STATS) THEN
  ! OPENING THE FILE
  WRITE(CLCSVNAME, '(A,I0,A)') 'gstats.', MYPROC_STATS, '.csv'
  IF (NPROC_STATS <= 1) WRITE(KULOUT, '(A,A)')    "GSTATS writing file ", CLCSVNAME
  IF (NPROC_STATS  > 1) WRITE(KULOUT, '(A,I0,A)') "GSTATS writing files gstats.{1..", NPROC_STATS, "}.csv"
  OPEN (NEWUNIT=ICSVUN, FILE=CLCSVNAME, ACTION='write')
  !write header
  ...
  DO JNUM=0,JPMAXSTAT
    ...
    IF (NCALLS(JNUM) > 1) THEN   
      ...
      ELSE
        ZSTDDEV= 0.0_JPRD
      ENDIF
      WRITE(CLCSVCOLS(1), '(I0)') JNUM
      CLCSVCOLS(2) = CCDESC(JNUM)
      WRITE(CLCSVCOLS(3), '(I0)') ICALLS
      WRITE(CLCSVCOLS(4), '(F0.6)') TIMESUM(JNUM)
      WRITE(CLCSVCOLS(5), '(F0.6)') TIMEMAX(JNUM)*1000._JPRD
      WRITE(CLCSVCOLS(6), '(F0.6)') TIMESUM(JNUM)/ICALLS*1000._JPRD
      WRITE(CLCSVCOLS(7), '(F0.6)') TTCPUSUM(JNUM)/ICALLS*1000._JPRD
      WRITE(CLCSVCOLS(8), '(F0.6)') TVCPUSUM(JNUM)/ICALLS*1000._JPRD
      WRITE(CLCSVCOLS(9), '(F0.6)') ZSTDDEV
      ...
      IF (LSTATS_MPL .AND. JNUM > 500 .AND. JNUM < 1001) THEN
        ...
        ELSE
          AVGRECVLEN=0.0_JPRD
        ENDIF
        WRITE(CLCSVCOLS(10), '(I0)') NUMSEND(JNUM)
        WRITE(CLCSVCOLS(11), '(F0.6)') SENDBYTES(JNUM)
        WRITE(CLCSVCOLS(12), '(F0.6)') AVGSENDLEN
        WRITE(CLCSVCOLS(13), '(I0)') NUMRECV(JNUM)
        WRITE(CLCSVCOLS(14), '(F0.6)') RECVBYTES(JNUM)
        WRITE(CLCSVCOLS(15), '(F0.6)') AVGRECVLEN
      ENDIF
      ! WRITE TO CSV FILE
      DO JJ = 1, 14
        WRITE(ICSVUN, '(2A)', advance='no') TRIM(CLCSVCOLS(JJ)), ','
      ENDDO
      WRITE(ICSVUN, '(A)') TRIM(CLCSVCOLS(15))
    ENDIF
  ENDDO
      ...
      DO JNUM=501,1000
        IF((NUMSEND(JNUM) /= 0 .OR. NUMRECV(JNUM) /= 0 ) &
           ...
           WRITE(KULOUT,'(I6,1X,A40,f6.1,2(I8,3F8.1))') &
            ...
            &  NUMSEND(JNUM),AVGSENDLEN,SENDBYTES(JNUM)*1.E-6_JPRD, SENDRATE, &
            &  NUMRECV(JNUM),AVGRECVLEN,RECVBYTES(JNUM)*1.E-6_JPRD, RECVRATE
        ENDIF
      ENDDO
      ...
      DO JNUM=501,1000
        IF(UNKNOWN_NUMSEND(JNUM) /= 0 .OR. UNKNOWN_NUMRECV(JNUM) /= 0 ) THEN
           WRITE(KULOUT,'(I6,1X,A40,2(I8,F8.1))') &
            &  JNUM,CCDESC(JNUM),&
            &  UNKNOWN_NUMSEND(JNUM),UNKNOWN_SENDBYTES(JNUM)*1.E-6_JPRD, &
            &  UNKNOWN_NUMRECV(JNUM),UNKNOWN_RECVBYTES(JNUM)*1.E-6_JPRD
        ENDIF
      ENDDO
      WRITE(KULOUT,'(7x,"TOTAL",35x,2(I8,F8.1),//)') &
       & SUM(UNKNOWN_NUMSEND(:)),SUM(UNKNOWN_SENDBYTES(:))*1.E-6_JPRD , &
       & SUM(UNKNOWN_NUMRECV(:)),SUM(UNKNOWN_RECVBYTES(:))*1.E-6_JPRD
  ...
  IF(LSTATS_MPL) THEN
    ...
    DO JNUM=501,1000
       IF((TOTSENDBYTES(JNUM).GT.0.0_JPRD.OR.TOTRECVBYTES(JNUM).GT.0.0_JPRD) &
          ...
          WRITE(KULOUT,'(I6,1X,A40,f6.1,2F10.1,F8.1)') &
           ...
           &  TOTRECVBYTES(JNUM)*1.E-9_JPRD , &
           &  (TOTSENDBYTES(JNUM)*1.E-9_JPRD)/MAXCOMMTIME(JNUM)
       ENDIF
    ENDDO
    ...
    IF( TOTSENDBYTESSUM.GT.0.0_JPRD ) THEN
      WRITE(KULOUT,'(/,A,42x,f6.1,2F10.1,F8.1)') &
       &   'TOTAL', &
       &   SUM(MAXCOMMTIME) , &
       &   TOTSENDBYTESSUM*1.E-9_JPRD, &
       &   TOTRECVBYTESSUM*1.E-9_JPRD, &
       &   (TOTSENDBYTESSUM*1.E-9_JPRD)/SUM(MAXCOMMTIME)
     ENDIF
  ENDIF
