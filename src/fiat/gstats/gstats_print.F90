...
! every rank writes its own csv output file
IF(LCSV_STATS)THEN
  ICSVUN = 45+MYPROC_STATS
  CSVCOLS='NaN'
  WRITE(*,*) "ME ". MYPROC_STATS, "IS WRITING IN ", ICSVUN, CSVNAME
  WRITE(CSVNAME, '(A,I0,A)') 'gstats.', MYPROC_STATS, '.csv'
  WRITE(*,*) "ME ". MYPROC_STATS, "IS OPENING ", ICSVUN, CSVNAME
...
IF (LSTATS .AND. MYPROC_STATS /= 1) THEN
  ...
  DO JNUM=0,JPMAXSTAT
    ...
    JJ = JJ+JPARRAYS
    IF (LCSV_STATS .AND. NCALLS(JNUM) > 1) THEN
...
ENDIF
IF (LCSV_STATS) THEN
  WRITE(*,*) "ME ". MYPROC_STATS, "IS CLOSING ", ICSVUN, CSVNAME
  CLOSE(ICSVUN) 
ENDIF
