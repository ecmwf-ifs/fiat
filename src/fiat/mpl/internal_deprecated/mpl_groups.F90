...
INTEGER(KIND=JPIM),SAVE :: ICOMM_PARENT=-1_JPIM
CONTAINS
! ------------------------------------------------------------------
...
IF (LGROUPSETUP) THEN
  ...
  ! Communicator has changed; free existing sub-communicators before rebuilding.
  CALL MPL_COMM_FREE(MPL_ALL_MS_COMM,   IERR)
  CALL MPL_COMM_FREE(MPL_ALL_LEVS_COMM, IERR)
  CALL MPL_COMM_FREE(MPL_COMM_GRID,     IERR)
  LGROUPSETUP = .FALSE.
END IF
