...
USE MPL_DATA_MODULE, ONLY : MPL_NUMPROC, LINITMPI_VIA_MPL, LINITMPL_ALLOCATOR_VIA_MPL, &
  &                         MPL_ATTACHED_BUFFER, JP_ATTACHED_BUFFER_BYTES
USE MPL_ALLOCATOR_MOD, ONLY : MPL_ALLOCATOR_FINAL, MPL_ALLOCATOR_INITIALIZED
USE MPL_MESSAGE_MOD, ONLY : MPL_MESSAGE
USE MPL_ALLOCATOR_MOD, ONLY : MPL_ALLOCATOR_FINAL
...
IF (ALLOCATED(MPL_ATTACHED_BUFFER)) THEN
  ...
  ELSE
    ...
    CALL MPI_BUFFER_DETACH(MPL_ATTACHED_BUFFER_ADDRESS,IBUFFMPI,IERROR)
    IF(PRESENT(KERROR)) THEN
      KERROR=IERROR
    ELSE
      IF( IERROR /= 0 )THEN
        CALL MPL_MESSAGE(CDMESSAGE='MPL_END ',KERROR=IERROR,LDABORT=LLABORT)
      ENDIF
    ENDIF
  ENDIF
...
IF (LINITMPL_ALLOCATOR_VIA_MPL .AND. MPL_ALLOCATOR_INITIALIZED()) THEN
