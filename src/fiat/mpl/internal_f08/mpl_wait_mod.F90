...
USE MPL_MPI,                  ONLY : MPI_REQUEST, MPI_STATUS, MPI_BYTE, MPI_WAITALL, MPI_WAIT, MPI_GET_COUNT
...
INTERFACE MPL_WAIT
MODULE PROCEDURE MPL_WAITS, MPL_WAITS_INTEGERS, MPL_WAIT1, MPL_WAIT1_INTEGER
END INTERFACE
...
SUBROUTINE MPL_WAITS_INTEGERS(KREQUEST,KOUNT,KBYTES,KERROR,CDSTRING)
...
TYPE(MPI_REQUEST)  :: IREQUESTS_LOCAL(SIZE(KREQUEST))

IREQUESTS_LOCAL(:)%MPI_VAL=KREQUEST(:)

CALL MPL_WAITS(IREQUESTS_LOCAL, CDSTRING=CDSTRING, KOUNT=KOUNT, KBYTES=KBYTES, KERROR=KERROR)

KREQUEST(:)=IREQUESTS_LOCAL(:)%MPI_VAL

END SUBROUTINE MPL_WAITS_INTEGERS
...
TYPE(MPI_REQUEST), INTENT(INOUT)        :: KREQUEST(:)
CHARACTER(LEN=*),  INTENT(IN), OPTIONAL :: CDSTRING
INTEGER(KIND=JPIM),INTENT(OUT),OPTIONAL :: KOUNT(SIZE(KREQUEST))
INTEGER(KIND=JPIM),INTENT(IN), OPTIONAL :: KBYTES
INTEGER(KIND=JPIM),INTENT(OUT),OPTIONAL :: KERROR
...
SUBROUTINE MPL_WAIT1_INTEGER(KREQUEST,KOUNT,KBYTES,KERROR,CDSTRING)
...
CALL MPL_WAIT1(IREQUEST_LOCAL, CDSTRING=CDSTRING, KOUNT=KOUNT, KBYTES=KBYTES, KERROR=KERROR)

KREQUEST=IREQUEST_LOCAL%MPI_VAL

END SUBROUTINE MPL_WAIT1_INTEGER
...
IF(PRESENT(KERROR))THEN
   KERROR=IWAITERR+ICOUNTERR
ELSE IF(IWAITERR /= 0) THEN
  CALL MPL_MESSAGE(CDMESSAGE='MPL_WAIT_WAITING',CDSTRING=CDSTRING,KERROR=IWAITERR,LDABORT=LLABORT)
ELSE IF(ICOUNTERR /= 0) THEN
