...
USE MPL_MPIF,        ONLY : MPI_STATUS, MPI_COMM, MPI_ANY_SOURCE, MPI_CHARACTER, MPI_ANY_TAG
USE MPL_DATA_MODULE, ONLY : MPL_NUMPROC, MPL_COMM_OML
...
INTEGER(KIND=JPIM) :: ITAG,ISOURCE,IERROR
TYPE(MPI_COMM)     :: ICOMM
LOGICAL :: LLWAIT,LLABORT=.TRUE.
...
ELSE
  IF(PRESENT(LDFLAG)) THEN
    ...
    ENDIF
  ELSE
    CALL MPL_MESSAGE(CDMESSAGE='MPL_PROBE: MUST PROVIDE LDFLAG ',CDSTRING=CDSTRING, &
...
IF(PRESENT(KERROR)) THEN
  KERROR=IERROR
ELSE
  IF(IERROR /= 0 ) CALL MPL_MESSAGE(CDMESSAGE='MPL_PROBE',CDSTRING=CDSTRING,KERROR=IERROR,LDABORT=LLABORT)
ENDIF
