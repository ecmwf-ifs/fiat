...
USE MPL_MPI,         ONLY : MPI_REQUEST, MPI_STATUS, MPI_TESTSOME, MPI_WAITSOME
...
TYPE(MPI_REQUEST)  :: KARRAY_OF_REQUESTS_LOCAL(SIZE(KARRAY_OF_REQUESTS))
INTEGER(KIND=JPIM) :: IERROR
INTEGER(KIND=JPIM) :: IINCOUNT
INTEGER(KIND=JPIM) :: IOUTCOUNT
TYPE(MPI_STATUS)   :: IARRAY_OF_STATUSES(SIZE(KARRAY_OF_REQUESTS))
...
KARRAY_OF_INDICES = -1
KARRAY_OF_REQUESTS_LOCAL(:)%MPI_VAL=KARRAY_OF_REQUESTS(:)

LLWAIT = .FALSE.
IF (PRESENT (LDWAIT)) LLWAIT = LDWAIT

IF (LLWAIT) THEN
  CALL MPI_WAITSOME(IINCOUNT, KARRAY_OF_REQUESTS_LOCAL, IOUTCOUNT, &
                    KARRAY_OF_INDICES, IARRAY_OF_STATUSES, IERROR)
ELSE
  CALL MPI_TESTSOME(IINCOUNT, KARRAY_OF_REQUESTS_LOCAL, IOUTCOUNT, &
                    KARRAY_OF_INDICES, IARRAY_OF_STATUSES, IERROR)
ENDIF
...
IF (PRESENT (KERROR)) THEN
  KERROR = IERROR
ELSE
  IF(IERROR /= 0) CALL MPL_MESSAGE(CDMESSAGE='MPL_TESTSOME',CDSTRING=CDSTRING,KERROR=IERROR,LDABORT=LLABORT)
ENDIF
