...
SUBMODULE (MPL_ALLREDUCE_MOD) MPL_ALLREDUCE_IMPLEMENTATION_MOD
...
USE MPL_MPI,           ONLY : MPI_COMM, MPI_OP, MPI_REQUEST, MPI_MIN, MPI_MAX, MPI_SUM, MPI_BXOR, MPI_ALLREDUCE, &
 &                            MPI_INTEGER, MPI_INTEGER8, MPI_REAL4, MPI_REAL8
USE MPL_DATA_MODULE,   ONLY : MPL_COMM_OML, MPL_OUTPUT, MPL_UNIT, MPL_RANK, MPL_NUMPROC, MPL_ERRUNIT, &
...
MODULE PROCEDURE MPL_ALLREDUCE_INT_SCALAR!(KSENDBUF,CDOPER,LDREPROD, &
...
END PROCEDURE MPL_ALLREDUCE_INT_SCALAR

MODULE PROCEDURE MPL_ALLREDUCE_INT8_SCALAR!(KSENDBUF,CDOPER,LDREPROD, &
!                            & KCOMM,KERROR,CDSTRING)
...
END PROCEDURE MPL_ALLREDUCE_INT8_SCALAR



MODULE PROCEDURE MPL_ALLREDUCE_REAL8_SCALAR!(PSENDBUF,CDOPER,LDREPROD, &
!                            & KCOMM,KERROR,CDSTRING)
...
END PROCEDURE MPL_ALLREDUCE_REAL8_SCALAR


MODULE PROCEDURE MPL_ALLREDUCE_REAL4_SCALAR!(PSENDBUF,CDOPER,LDREPROD, &
!                            & KCOMM,KERROR,CDSTRING)
...
END PROCEDURE MPL_ALLREDUCE_REAL4_SCALAR


MODULE PROCEDURE MPL_ALLREDUCE_INT!(KSENDBUF,CDOPER,LDREPROD, &
!                            & KCOMM,KERROR,CDSTRING)
...
!!INTEGER(KIND=JPIM),INTENT(OUT),OPTIONAL :: KERROR
!!CHARACTER(LEN=*),INTENT(IN),OPTIONAL :: CDSTRING
INTEGER(KIND=JPIM), ALLOCATABLE :: IRECVBUF(:)
INTEGER(KIND=JPIM) :: ISENDCOUNT,IERROR
TYPE(MPI_COMM)     :: ICOMM
TYPE(MPI_OP)       :: IOPER
INTEGER(KIND=JPIM) :: ITID
ITID = OML_MY_THREAD()
IERROR = 0
ALLOCATE(IRECVBUF(SIZE(KSENDBUF)))
...
ELSEIF(CDOPER(1:4) == 'XOR' .OR. CDOPER(1:4) == 'xor' ) THEN
  IOPER = MPI_BXOR
ELSE
  CALL MPL_MESSAGE(CDMESSAGE='MPL_ALLREDUCE: ERROR UNKNOWN OPERATOR',&
...
IF(PRESENT(KERROR)) THEN
  KERROR=IERROR
ELSE
  IF(IERROR /= 0 ) CALL MPL_MESSAGE(CDMESSAGE='MPL_ALLREDUCE',CDSTRING=CDSTRING,KERROR=IERROR,LDABORT=LLABORT)
ENDIF
...
END PROCEDURE MPL_ALLREDUCE_INT

MODULE PROCEDURE MPL_ALLREDUCE_INT8!(KSENDBUF,CDOPER,LDREPROD, &
!                            & KCOMM,KERROR,CDSTRING)
!!INTEGER(KIND=JPIB),INTENT(INOUT)     :: KSENDBUF(:)
!!CHARACTER(LEN=*),INTENT(IN)    :: CDOPER
!!LOGICAL,INTENT(IN),OPTIONAL :: LDREPROD
...
IF(LMPLSTATS) THEN
  CALL MPL_SENDSTATS(ISENDCOUNT,MPI_INTEGER8%MPI_VAL)
  CALL MPL_RECVSTATS(ISENDCOUNT,MPI_INTEGER8%MPI_VAL)
ENDIF
...
END PROCEDURE MPL_ALLREDUCE_INT8


MODULE PROCEDURE MPL_ALLREDUCE_REAL8!(PSENDBUF,CDOPER,LDREPROD, &
!                            & KCOMM,KERROR,CDSTRING)
...
!!INTEGER(KIND=JPIM),INTENT(OUT),OPTIONAL :: KERROR
!!CHARACTER(LEN=*),INTENT(IN),OPTIONAL :: CDSTRING
REAL(KIND=JPRD), ALLOCATABLE :: ZRECVBUF(:)
...
TYPE(MPI_REQUEST)  :: ISREQ(MPL_NUMPROC)
INTEGER(KIND=JPIM) :: ITID
IERROR = 0
ITID = OML_MY_THREAD()
LLREPRODSUM = .FALSE.
ALLOCATE(ZRECVBUF(SIZE(PSENDBUF)))
...
ELSEIF(CDOPER(1:3) == 'SUM' .OR. CDOPER(1:3) == 'sum' ) THEN
  ...
  ELSE
    CALL MPL_MESSAGE( CDMESSAGE='MPL_ALLREDUCE: SUMMATION OPERATOR NOT REPRODUCIBLE IN REAL MODE',&
     & CDSTRING=CDSTRING,KERROR=IERROR,LDABORT=LLABORT)
  ENDIF
ELSE
  CALL MPL_MESSAGE(CDMESSAGE='MPL_ALLREDUCE: ERROR UNKNOWN OPERATOR',&
...
!    WRITE(0,*) 'STAGE ',JSTAGE
    ...
    IF(ISEND > 0 .AND. MPL_RANK <= II) THEN
      IMSENT=IMSENT+1
      CALL MPL_SEND(PSENDBUF,KDEST=ISEND,KCOMM=ICOMM%MPI_VAL,KTAG=ITAG,KERROR=IERROR,&
       &KMP_TYPE=JP_NON_BLOCKING_STANDARD,KREQUEST=ISREQ(IMSENT)%MPI_VAL,CDSTRING='MPLS_SEND')
!      write(0,*) 'I SEND TO ',MPL_RANK,ISEND
    ENDIF
    IRECV=MPL_RANK + IHALF
    IF(IRECV <=MPL_NUMPROC .AND. MPL_RANK <= IHALF) THEN
      CALL MPL_RECV(ZRECVBUF,KSOURCE=IRECV,KCOMM=ICOMM%MPI_VAL,KTAG=ITAG,&
...
!      write(0,*) 'I RECV FROM ',MPL_RANK,IRECV
  ...
  ENDDO
  IF(IMSENT > 0) THEN
    CALL MPL_WAIT(ISREQ(1:IMSENT),CDSTRING='MPLS_SEND')
  ENDIF
  IF (MPL_RANK == 1) THEN
    ZRECVBUF(:) = PSENDBUF(:)
  ENDIF
!  write(0,*) 'enter broadcast '
  CALL MPL_BROADCAST(ZRECVBUF,ITAG,KCOMM=ICOMM%MPI_VAL,KROOT=1,KERROR=IERROR)
...
END PROCEDURE MPL_ALLREDUCE_REAL8

MODULE PROCEDURE MPL_ALLREDUCE_REAL8_2D!(PSENDBUF,CDOPER,LDREPROD, &
!                            & KCOMM,KERROR,CDSTRING)
...
IF(PRESENT(KERROR)) THEN
  KERROR=IERROR
ELSE
  IF(IERROR /= 0 ) CALL MPL_MESSAGE(CDMESSAGE='MPL_ALLREDUCE',CDSTRING=CDSTRING, &
...
END PROCEDURE MPL_ALLREDUCE_REAL8_2D


MODULE PROCEDURE MPL_ALLREDUCE_REAL4!(PSENDBUF,CDOPER,LDREPROD, &
!                            & KCOMM,KERROR,CDSTRING)
...
!!INTEGER(KIND=JPIM),INTENT(OUT),OPTIONAL :: KERROR
!!CHARACTER(LEN=*),INTENT(IN),OPTIONAL :: CDSTRING
REAL(KIND=JPRM), ALLOCATABLE :: ZRECVBUF(:)
...
!-- Near reproducible summation
  ...
  IF (MPL_RANK == 1) THEN
    DO IPROC=2,MPL_NUMPROC
      CALL MPL_RECV(ZRECVBUF,KSOURCE=IPROC,KCOMM=ICOMM%MPI_VAL,KTAG=ITAG,&
        &KERROR=IERROR,KOUNT=ICOUNT)
      IF (ICOUNT /= ISENDCOUNT) THEN
        ...
        CALL MPL_MESSAGE(CDMESSAGE='MPL_ALLREDUCE',CDSTRING=CDSTRING,KERROR=IERROR,LDABORT=LLABORT)
      ENDIF
      PSENDBUF(:) = PSENDBUF(:) + ZRECVBUF(:)
    ENDDO
    ZRECVBUF(:) = PSENDBUF(:)
  ELSE
    CALL MPL_SEND(PSENDBUF,KDEST=1,KCOMM=ICOMM%MPI_VAL,KTAG=ITAG,KERROR=IERROR,&
      &KMP_TYPE=JP_BLOCKING_STANDARD,CDSTRING='MPLS_SEND')
  ENDIF
  ITAG = ITAG + 1
  CALL MPL_BROADCAST(ZRECVBUF,KTAG=ITAG,KCOMM=ICOMM%MPI_VAL,KROOT=1,KERROR=IERROR)
ELSE  
  IF ( MPL_NUMPROC > 1 ) &
  ...
  IF(LMPLSTATS) THEN
    ...
    CALL MPL_RECVSTATS(ISENDCOUNT,MPI_REAL4%MPI_VAL)
  ENDIF
...
END PROCEDURE MPL_ALLREDUCE_REAL4

MODULE PROCEDURE MPL_ALLREDUCE_REAL4_2D!(PSENDBUF,CDOPER,LDREPROD, &
!                            & KCOMM,KERROR,CDSTRING)
...
ELSEIF(CDOPER(1:3) == 'SUM' .OR. CDOPER(1:3) == 'sum' ) THEN
  ...
  ELSE
    CALL MPL_MESSAGE(&
     & CDMESSAGE='MPL_ALLREDUCE: SUMMATION OPERATOR NOT REPRODUCIBLE IN REAL MODE',&
     & CDSTRING=CDSTRING,KERROR=IERROR,LDABORT=LLABORT)
  ENDIF
ELSE
  CALL MPL_MESSAGE(&
   & CDMESSAGE='MPL_ALLREDUCE: ERROR UNKNOWN OPERATOR',&
   & CDSTRING=CDSTRING,KERROR=IERROR,LDABORT=LLABORT)
ENDIF
...
IF(PRESENT(KERROR)) THEN
  KERROR=IERROR
ELSE
  IF(IERROR /= 0 ) CALL MPL_MESSAGE(CDMESSAGE='MPL_ALLREDUCE',&
 &                                  CDSTRING=CDSTRING,KERROR=IERROR,LDABORT=LLABORT)
ENDIF
...
END PROCEDURE MPL_ALLREDUCE_REAL4_2D

END SUBMODULE MPL_ALLREDUCE_IMPLEMENTATION_MOD
