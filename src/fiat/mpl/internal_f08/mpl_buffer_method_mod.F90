...
USE MPL_DATA_MODULE, ONLY : MPL_NUMPROC, MPL_ATTACHED_BUFFER, MPL_MBX_SIZE, MPL_METHOD, MPL_RANK, MPL_UNIT, MPL_IDS, &
 &                          JP_BLOCKING_STANDARD, JP_BLOCKING_BUFFERED, JP_ATTACHED_BUFFER_BYTES
USE MPL_MESSAGE_MOD, ONLY : MPL_MESSAGE
USE MPL_MPI,         ONLY : MPI_BUFFER_ATTACH, MPI_BUFFER_DETACH
...
IF (ALLOCATED(MPL_ATTACHED_BUFFER)) THEN
  CALL MPI_BUFFER_DETACH(MPL_ATTACHED_BUFFER_ADDRESS,MPL_MBX_SIZE,IERROR)
...
!    convert to bytes
  ...
  ELSE
    IF( IERROR /= 0 )THEN
      CALL MPL_MESSAGE(CDMESSAGE='MPL_BUFFER_METHOD ',CDSTRING='MPI_BUFFER_ATTACH ERROR',KERROR=IERROR,LDABORT=LLABORT)
    ENDIF
  ENDIF
ELSE
!    invalid type
  IF(PRESENT(KERROR)) THEN
    KERROR=1
  ELSE
    CALL MPL_MESSAGE(CDMESSAGE='MPL_BUFFER_METHOD',CDSTRING='INVALID KMP_TYPE=',KERROR=KMP_TYPE,LDABORT=LLABORT)
  ENDIF
ENDIF
