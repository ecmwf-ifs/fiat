...
USE MPL_MPIF,        ONLY : MPI_DATATYPE, MPI_TYPE_SIZE
USE YOMMPLSTATS,     ONLY : LMPLSTATS, MPLSENDBYTES, MPLRECVBYTES, MPLSENDNUM, MPLRECVNUM
...
TYPE(MPI_DATATYPE) :: ITYPE_LOCAL
INTEGER(KIND=JPIM) :: ISIZE,IERR,ITH,OMP_GET_THREAD_NUM

ITYPE_LOCAL%MPI_VAL=ITYPE
ITH = 0
!$ ITH = OMP_GET_THREAD_NUM()

MPLSENDNUM(ITH) = MPLSENDNUM(ITH) + 1

CALL MPI_TYPE_SIZE(ITYPE_LOCAL,ISIZE,IERR)
