...
USE MPL_MPI,         ONLY : MPI_COMM, MPI_COMM_RANK
USE MPL_DATA_MODULE, ONLY : MPL_RANK, MPL_NUMPROC, MPL_RANK
...
IF(PRESENT(KCOMM)) THEN
  ...
  CALL MPI_COMM_RANK(KCOMM_LOCAL, IRANK, IERROR)
  IF(IERROR /= 0 ) CALL MPL_MESSAGE(CDMESSAGE='MPL_MYRANK: ERROR FROM MPI_COMM_RANK',KERROR=IERROR)
  MPL_MYRANK=IRANK+1
ELSE
  MPL_MYRANK=MPL_RANK
ENDIF
