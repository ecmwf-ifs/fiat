...
#ifndef MPI1
CALL MPI_WAITANY(IREQLEN,KREQUEST_LOCAL,KINDEX,IWAIT_STATUS,IWAITERR)
#else
...
DO J=1,IREQLEN
   IF (KREQUEST_LOCAL(J) /= MPI_REQUEST_NULL) THEN
      CALL MPI_WAIT(KREQUEST_LOCAL(J),IWAIT_STATUS,IWAITERR)
      KINDEX = J
      EXIT
   ENDIF
ENDDO
