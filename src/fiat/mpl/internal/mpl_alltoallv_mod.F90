...
SUBROUTINE MPL_ALLTOALLV_PREAMB(KSENDCOUNTS,KISENDDISPL,&
 & KRECVCOUNTS,KIRECVDISPL,KSENDDISPL,KRECVDISPL,KMP_TYPE,KCOMM,KREQUEST,CDSTRING)
...
IF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
  IF(.NOT.PRESENT(KREQUEST)) &
 &       CALL MPL_MESSAGE(CDMESSAGE='MPL_ALLTOALLV',CDSTRING=' KREQUEST MISSING', LDABORT=LLABORT)
ENDIF
...
CALL MPL_ALLTOALLV_PREAMB(KSENDCOUNTS,ISENDDISPL,&
 & KRECVCOUNTS,IRECVDISPL,KSENDDISPL,KRECVDISPL,KMP_TYPE,KCOMM,KREQUEST,CDSTRING)

IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
  CALL MPI_ALLTOALLV(PSENDBUF(:),KSENDCOUNTS,ISENDDISPL,MPI_REAL8, &
   & PRECVBUF(:),KRECVCOUNTS,IRECVDISPL,MPI_REAL8,ICOMM,IERROR)
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
  CALL MPI_IALLTOALLV(PSENDBUF(:),KSENDCOUNTS,ISENDDISPL,MPI_REAL8, &
   & PRECVBUF(:),KRECVCOUNTS,IRECVDISPL,MPI_REAL8,ICOMM,IREQUEST_LOCAL,IERROR)
...
IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
  CALL MPI_ALLTOALLV(PSENDBUF(:),KSENDCOUNTS,ISENDDISPL,MPI_REAL4, &
   & PRECVBUF(:),KRECVCOUNTS,IRECVDISPL,MPI_REAL4,ICOMM,IERROR)
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
  CALL MPI_IALLTOALLV(PSENDBUF(:),KSENDCOUNTS,ISENDDISPL,MPI_REAL4, &
   & PRECVBUF(:),KRECVCOUNTS,IRECVDISPL,MPI_REAL4,ICOMM,IREQUEST_LOCAL,IERROR)
  KREQUEST=IREQUEST_LOCAL%MPI_VAL
ELSE
  IF(PRESENT(KERROR)) THEN
    IERROR=1
...
IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
  CALL MPI_ALLTOALLV(KSENDBUF(1),KSENDCOUNTS,ISENDDISPL,MPI_INTEGER, &
   & KRECVBUF(1),KRECVCOUNTS,IRECVDISPL,MPI_INTEGER,ICOMM,IERROR)
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
  CALL MPI_IALLTOALLV(KSENDBUF(1),KSENDCOUNTS,ISENDDISPL,MPI_INTEGER, &
   & KRECVBUF(1),KRECVCOUNTS,IRECVDISPL,MPI_INTEGER,ICOMM,IREQUEST_LOCAL,IERROR)
  KREQUEST=IREQUEST_LOCAL%MPI_VAL
ELSE
  IF(PRESENT(KERROR)) THEN
    IERROR=1
