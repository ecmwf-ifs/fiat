...
ELSE
  IF(PRESENT(KREQUEST)) THEN
     CALL DISPL_ARRAY_LIST%APPEND(RECV=KRECVCOUNTS) ! needs only the size of krecvcounts
...
ELSE
   IF(PRESENT(KREQUEST)) THEN
     CALL DISPL_ARRAY_LIST%APPEND(SEND=KSENDCOUNTS, NO_NEW_NODE=.TRUE.)
...
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
  ...
  IF (.NOT. PRESENT(KSENDDISPL)) THEN
    CALL DISPL_ARRAY_LIST%APPEND(REQ=KREQUEST,NO_NEW_NODE=.TRUE.)
  END IF
ELSE
...
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
   ...
   CALL MPI_IALLTOALLV(KSENDBUF(1),KSENDCOUNTS,KISENDDISPL_PT,INT(MPI_INTEGER), &
        & KRECVBUF(1),KRECVCOUNTS,KIRECVDISPL_PT,INT(MPI_INTEGER),ICOMM,KREQUEST,IERROR)
   IF (.NOT. PRESENT(KSENDDISPL)) THEN
     CALL DISPL_ARRAY_LIST%APPEND(REQ=KREQUEST,NO_NEW_NODE=.TRUE.)
   END IF
ELSE
