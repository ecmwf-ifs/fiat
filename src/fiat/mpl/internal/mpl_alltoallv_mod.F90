...
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
   ...
   IF(.NOT.PRESENT(KSENDDISPL)) THEN
      CALL DISPL_ARRAY_LIST%APPEND(ISENDDISPL,IRECVDISPL,KREQUEST)
      CALL MPI_IALLTOALLV(KSENDBUF(1),KSENDCOUNTS,DISPL_ARRAY_LIST%HEAD%SEND,INT(MPI_INTEGER), &
           & KRECVBUF(1),KRECVCOUNTS,DISPL_ARRAY_LIST%HEAD%recv,INT(MPI_INTEGER),ICOMM,KREQUEST,IERROR)
      ELSE
