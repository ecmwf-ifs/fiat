...
IF(PRESENT(KRECVDISPL)) THEN
  ...
  !ENDDO
ELSE
  IF(PRESENT(KREQUEST)) THEN
     CALL DISPL_ARRAY_LIST%APPEND(KREQUEST,RECV=KRECVCOUNTS) ! needs only the size of krecvcounts
     KIRECVDISPL_PT => DISPL_ARRAY_LIST%HEAD%RECV
  ELSE
...
ELSE
   IF(PRESENT(KREQUEST)) THEN
     CALL DISPL_ARRAY_LIST%APPEND(KREQUEST,SEND=KSENDCOUNTS)
     KISENDDISPL_PT => DISPL_ARRAY_LIST%HEAD%SEND
  ELSE
...
IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
  CALL MPI_ALLTOALLV(PSENDBUF(:),KSENDCOUNTS,KISENDDISPL_PT,INT(MPI_REAL8), &
   & PRECVBUF(:),KRECVCOUNTS,KIRECVDISPL_PT,INT(MPI_REAL8),ICOMM,IERROR)
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
  ...
  CALL MPI_IALLTOALLV(PSENDBUF(:),KSENDCOUNTS,KISENDDISPL_PT,INT(MPI_REAL8), &
   & PRECVBUF(:),KRECVCOUNTS,KIRECVDISPL_PT,INT(MPI_REAL8),ICOMM,KREQUEST,IERROR)
...
IERROR=0
CALL MPL_ALLTOALLV_PREAMB(KSENDCOUNTS,ISENDDISPL,KISENDDISPL_PT,&
 & KRECVCOUNTS,IRECVDISPL,KISENDDISPL_PT,KSENDDISPL,KRECVDISPL,KMP_TYPE,KCOMM,KREQUEST,CDSTRING)
IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
  CALL MPI_ALLTOALLV(PSENDBUF(1),KSENDCOUNTS,KISENDDISPL_PT,INT(MPI_REAL4), &
   & PRECVBUF(:),KRECVCOUNTS,KIRECVDISPL_PT,INT(MPI_REAL4),ICOMM,IERROR)
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
  CALL MPI_IALLTOALLV(PSENDBUF(1),KSENDCOUNTS,KISENDDISPL_PT,INT(MPI_REAL4), &
   & PRECVBUF(:),KRECVCOUNTS,KIRECVDISPL_PT,INT(MPI_REAL4),ICOMM,KREQUEST,IERROR)
  !!! FIXME since the lifetime of IRECVDISPL and ISENDDISPL is limited to this routine
  !!! we have to complete MPI_WAIT right in place
  !!! Details: https://github.com/ecmwf-ifs/fiat/issues/17
  CALL MPL_WAIT(KREQUEST)
...
CALL MPL_ALLTOALLV_PREAMB(KSENDCOUNTS,ISENDDISPL,KISENDDISPL_PT,&
 & KRECVCOUNTS,IRECVDISPL,KIRECVDISPL_PT,KSENDDISPL,KRECVDISPL,KMP_TYPE,KCOMM,KREQUEST,CDSTRING)
IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
  CALL MPI_ALLTOALLV(KSENDBUF(1),KSENDCOUNTS,KISENDDISPL_PT,INT(MPI_INTEGER), &
   & KRECVBUF(1),KRECVCOUNTS,KIRECVDISPL_PT,INT(MPI_INTEGER),ICOMM,IERROR)
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
   write(0,*) 's', mpl_rank, kisenddispl_pt
   write(0,*) 'r', mpl_rank, kirecvdispl_pt
   IF(.NOT.PRESENT(KREQUEST)) CALL MPL_MESSAGE(KERROR,'MPL_ALLTOALLV',' KREQUEST MISSING',LDABORT=LLABORT)
   CALL MPI_IALLTOALLV(KSENDBUF(1),KSENDCOUNTS,KISENDDISPL_PT,INT(MPI_INTEGER), &
        & KRECVBUF(1),KRECVCOUNTS,KIRECVDISPL_PT,ICOMM,KREQUEST,IERROR)
  !!! FIXME since the lifetime of IRECVDISPL and ISENDDISPL is limited to this routine
  !!! we have to complete MPI_WAIT right in place
  !!! Details: https://github.com/ecmwf-ifs/fiat/issues/17
  !CALL MPL_WAIT(KREQUEST)
