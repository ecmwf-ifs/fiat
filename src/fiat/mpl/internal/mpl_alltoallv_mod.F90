...
TYPE(MPI_REQUEST),  INTENT(OUT),OPTIONAL :: KREQUEST
...
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
  IF(.NOT.PRESENT(KREQUEST)) CALL MPL_MESSAGE(CDMESSAGE='MPL_ALLTOALLV',CDSTRING=' KREQUEST MISSING', &
   &                                          KERROR=KERROR,LDABORT=LLABORT)
  CALL MPI_IALLTOALLV(PSENDBUF(:),KSENDCOUNTS,ISENDDISPL,MPI_REAL8, &
   & PRECVBUF(:),KRECVCOUNTS,IRECVDISPL,MPI_REAL8,ICOMM,KREQUEST,IERROR)
ELSE
...
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
  CALL MPI_IALLTOALLV(PSENDBUF(:),KSENDCOUNTS,ISENDDISPL,MPI_REAL4, &
   & PRECVBUF(:),KRECVCOUNTS,IRECVDISPL,MPI_REAL4,ICOMM,KREQUEST,IERROR)
ELSE
  IF(PRESENT(KERROR)) THEN
    IERROR=1
  ELSE
...
ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
 ...
 &                                            KERROR=KERROR,LDABORT=LLABORT)
  CALL MPI_IALLTOALLV(KSENDBUF(1),KSENDCOUNTS,ISENDDISPL,MPI_INTEGER, &
   & KRECVBUF(1),KRECVCOUNTS,IRECVDISPL,MPI_INTEGER,ICOMM,KREQUEST,IERROR)
ELSE
  IF(PRESENT(KERROR)) THEN
    IERROR=1
  ELSE
