...
INTEGER(KIND=JPIM),TARGET,INTENT(IN) :: KRECVCOUNTS(:),KSENDCOUNTS(:)
...
ELSE
  IF(PRESENT(KREQUEST)) THEN
     CALL DISPL_ARRAY_LIST%APPEND(KREQUEST,KRECVCOUNTS) ! needs only the size
...
ELSE
   IF(PRESENT(KREQUEST)) THEN
     CALL DISPL_ARRAY_LIST%APPEND(KREQUEST,KSENDCOUNTS)
