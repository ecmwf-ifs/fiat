...
TYPE(MPI_REQUEST)  :: KREQUEST_LOCAL(SIZE(KREQUEST))
INTEGER(KIND=JPIM) :: IWAITERR,ICOUNTERR,JL,IREQLEN,ICOUNT,IW
TYPE(MPI_STATUS)   :: IWAIT_STATUS(SIZE(KREQUEST))
LOGICAL            :: LLABORT

LLABORT=.TRUE.
IWAITERR=0
ICOUNTERR=0
KREQUEST_LOCAL(:)%MPI_VAL=KREQUEST(:)
IF(MPL_NUMPROC < 1) CALL MPL_MESSAGE( &
  & CDMESSAGE='MPL_WAITS: MPL NOT INITIALISED ',LDABORT=LLABORT) 

IREQLEN=SIZE(KREQUEST)
#ifndef MPI1
CALL MPI_WAITALL(IREQLEN,KREQUEST_LOCAL,IWAIT_STATUS,IWAITERR)
#else
DO JL=1,IREQLEN
  CALL MPI_WAIT(KREQUEST_LOCAL(JL),IWAIT_STATUS(1,JL),IW)
...
IF(PRESENT(KOUNT))THEN
  ...
  ENDIF
  DO JL=1,IREQLEN
    CALL MPI_GET_COUNT(IWAIT_STATUS(JL),MPI_BYTE,KOUNT(JL),ICOUNTERR)
...
ELSE IF(IWAITERR /= 0) THEN
  CALL MPL_MESSAGE(CDMESSAGE='MPL_WAITS_WAITING',CDSTRING=CDSTRING,KERROR=IWAITERR,LDABORT=LLABORT)
ELSE IF(ICOUNTERR /= 0) THEN
  CALL MPL_MESSAGE(CDMESSAGE='MPL_WAITS_COUNT',CDSTRING=CDSTRING,KERROR=ICOUNTERR,LDABORT=LLABORT)
ENDIF
...
CALL MPI_WAIT(KREQUEST_LOCAL,IWAIT_STATUS,IWAITERR)
KREQUEST=KREQUEST_LOCAL%MPI_VAL
IF(PRESENT(KOUNT))THEN
  IF (.not.PRESENT(KBYTES)) THEN
    CALL MPL_MESSAGE( &
    ...
    & CDSTRING=CDSTRING,LDABORT=LLABORT)
  ENDIF
  CALL MPI_GET_COUNT(IWAIT_STATUS,MPI_BYTE,KOUNT,ICOUNTERR)
  KOUNT = KOUNT / KBYTES
ENDIF
...
IF(PRESENT(KERROR))THEN
  KERROR=IWAITERR+ICOUNTERR
ELSE IF(IWAITERR /= 0) THEN
  CALL MPL_MESSAGE(CDMESSAGE='MPL_WAIT_WAITING',CDSTRING=CDSTRING,LDABORT=LLABORT)
ELSE IF(ICOUNTERR /= 0) THEN
  CALL MPL_MESSAGE(CDMESSAGE='MPL_WAIT_COUNT',CDSTRING=CDSTRING,KERROR=ICOUNTERR,LDABORT=LLABORT)
ENDIF
