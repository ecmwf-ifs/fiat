...
#ifndef MPI1
CALL MPI_WAITALL(IREQLEN,IREQUEST_LOCAL,IWAIT_STATUS,IWAITERR)
#else
DO JL=1,IREQLEN
  CALL MPI_WAIT(IREQUEST_LOCAL(JL),IWAIT_STATUS(1,JL),IW)
...
CALL MPI_WAIT(IREQUEST_LOCAL,IWAIT_STATUS,IWAITERR)
KREQUEST=IREQUEST_LOCAL%MPI_VAL
IF(PRESENT(KOUNT))THEN
  IF (.not.PRESENT(KBYTES)) THEN
    CALL MPL_MESSAGE( &
