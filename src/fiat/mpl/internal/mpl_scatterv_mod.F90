...
ELSE
  IF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL DISPL_ARRAY_LIST%APPEND(NPROC=IPL_NUMPROC, RECV_PT=KISENDDISPL_PT)
  ELSE
...
INTEGER IDATA_TYPE
LOGICAL PRESENT_SENDBUF

IDATA_TYPE=INT(MPI_REAL8)
PRESENT_SENDBUF=PRESENT(PSENDBUF)
...
PRESENT_SENDBUF=PRESENT(KSENDBUF)
...
!!$     IF(.NOT. PRESENT(KSENDVDISPL)) THEN
