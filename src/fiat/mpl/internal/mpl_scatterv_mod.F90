...
TYPE(MPI_COMM)     :: ICOMM
INTEGER(KIND=JPIM) :: IROOT,IPL_NUMPROC,IRECVCOUNT,ISENDBUFSIZE,IR,IPL_MYRANK,IMP_TYPE
...
INTEGER(KIND=JPIM) :: ITID,ICOMM_SIZE,IERR
ITID = OML_MY_THREAD()
...
IF (IERR /= 0) THEN
  CALL ABOR1('PROBLEM WITH SIZE OF ICOMM IN MPL_SCATTERV_PREAMB1')
ELSE
  IF (.NOT. ALLOCATED(ONES)) THEN
    ALLOCATE(ONES(ICOMM_SIZE))
    ONES(:) = 1_JPIM
  ENDIF
ENDIF
...
ELSE
  ...
  IPL_MYRANK  = MPL_MYRANK(ICOMM%MPI_VAL)
ENDIF
...
IF(IPL_MYRANK == IROOT) THEN
  ...
  IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
    CALL MPI_SCATTERV(PSENDBUF,KSENDCOUNTS,ISENDDISPL,MPI_REAL8, &
     & PRECVBUF,IRECVCOUNT,MPI_REAL8,IROOT-1,ICOMM,IERROR)
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_ISCATTERV(PSENDBUF,KSENDCOUNTS,ISENDDISPL,MPI_REAL8, &
     & PRECVBUF,IRECVCOUNT,MPI_REAL8,IROOT-1,ICOMM,IREQUEST_LOCAL,IERROR)
    KREQUEST=IREQUEST_LOCAL%MPI_VAL
  ENDIF
  IF(LMPLSTATS) THEN
    CALL MPL_SENDSTATS(SUM(KSENDCOUNTS),MPI_REAL8%MPI_VAL)
    CALL MPL_RECVSTATS(IRECVCOUNT,MPI_REAL8%MPI_VAL)
  ENDIF
ELSE
  IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
    CALL MPI_SCATTERV(ZDUM,ONES,ONES,MPI_REAL8, &
     & PRECVBUF,IRECVCOUNT,MPI_REAL8,IROOT-1,ICOMM,IERROR)
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_ISCATTERV(ZDUM,ONES,ONES,MPI_REAL8, &
     & PRECVBUF,IRECVCOUNT,MPI_REAL8,IROOT-1,ICOMM,IREQUEST_LOCAL,IERROR)
    KREQUEST=IREQUEST_LOCAL%MPI_VAL
  ENDIF
  IF(LMPLSTATS) THEN
...
IF(IPL_MYRANK == IROOT) THEN
  ...
  CALL MPL_SCATTERV_PREAMB2(KSENDCOUNTS,ISENDDISPL,KSENDDISPL,CDSTRING)
  IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
    CALL MPI_SCATTERV(PSENDBUF,KSENDCOUNTS,ISENDDISPL,MPI_REAL4, &
     & PRECVBUF,IRECVCOUNT,MPI_REAL4,IROOT-1,ICOMM,IERROR)
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_ISCATTERV(PSENDBUF,KSENDCOUNTS,ISENDDISPL,MPI_REAL4, &
     & PRECVBUF,IRECVCOUNT,MPI_REAL4,IROOT-1,ICOMM,IREQUEST_LOCAL,IERROR)
    KREQUEST=IREQUEST_LOCAL%MPI_VAL
  ENDIF
  IF(LMPLSTATS) THEN
    CALL MPL_SENDSTATS(SUM(KSENDCOUNTS),MPI_REAL4%MPI_VAL)
    CALL MPL_RECVSTATS(IRECVCOUNT,MPI_REAL4%MPI_VAL)
  ENDIF
ELSE
  IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
    CALL MPI_SCATTERV(ZDUM_4,ONES,ONES,MPI_REAL4, &
     & PRECVBUF,IRECVCOUNT,MPI_REAL4,IROOT-1,ICOMM,IERROR)
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_ISCATTERV(ZDUM_4,ONES,ONES,MPI_REAL4, &
     & PRECVBUF,IRECVCOUNT,MPI_REAL4,IROOT-1,ICOMM,IREQUEST_LOCAL,IERROR)
    KREQUEST=IREQUEST_LOCAL%MPI_VAL
  ENDIF
  IF(LMPLSTATS) THEN
...
CALL MPL_SCATTERV_PREAMB1(KCOMM,KROOT,KMP_TYPE,IREQUEST_LOCAL%MPI_VAL)
...
IF(IPL_MYRANK == IROOT) THEN
  ...
  CALL MPL_SCATTERV_PREAMB2(KSENDCOUNTS,ISENDDISPL,KSENDDISPL,&
   & CDSTRING)
  IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
    CALL MPI_SCATTERV(KSENDBUF,KSENDCOUNTS,ISENDDISPL,MPI_INTEGER, &
     & KRECVBUF,IRECVCOUNT,MPI_INTEGER,IROOT-1,ICOMM,IERROR)
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_ISCATTERV(KSENDBUF,KSENDCOUNTS,ISENDDISPL,MPI_INTEGER, &
     & KRECVBUF,IRECVCOUNT,MPI_INTEGER,IROOT-1,ICOMM,IREQUEST_LOCAL,IERROR)
    KREQUEST=IREQUEST_LOCAL%MPI_VAL
  ENDIF
  IF(LMPLSTATS) THEN
    CALL MPL_SENDSTATS(SUM(KSENDCOUNTS),MPI_INTEGER%MPI_VAL)
    CALL MPL_RECVSTATS(IRECVCOUNT,MPI_INTEGER%MPI_VAL)
  ENDIF
ELSE
  IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
    CALL MPI_SCATTERV(ZDUM_INT,ONES,ONES,MPI_INTEGER, &
     & KRECVBUF,IRECVCOUNT,MPI_INTEGER,IROOT-1,ICOMM,IERROR)
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_ISCATTERV(ZDUM_INT,ONES,ONES,MPI_INTEGER, &
     & KRECVBUF,IRECVCOUNT,MPI_INTEGER,IROOT-1,ICOMM,IREQUEST_LOCAL,IERROR)
    KREQUEST=IREQUEST_LOCAL%MPI_VAL
  ENDIF
  IF(LMPLSTATS) THEN
