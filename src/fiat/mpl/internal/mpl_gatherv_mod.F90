...
CALL MPL_GATHERV_PREAMB1(IERROR,IPL_NUMPROC,IPL_MYRANK,LOCALCOMM%MPI_VAL,IROOT,IMP_TYPE,KCOMM,KROOT,KMP_TYPE,KREQUEST)
...
IF(IPL_MYRANK == IROOT) THEN
  ...
  IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
    CALL MPI_GATHERV(PSENDBUF,ISENDCOUNT,MPI_REAL4,PRECVBUF,KRECVCOUNTS, &
     &  IRECVDISPL,MPI_REAL4,IROOT-1,LOCALCOMM,IERROR)
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_IGATHERV(PSENDBUF,ISENDCOUNT,MPI_REAL4,PRECVBUF,KRECVCOUNTS, &
     &  IRECVDISPL,MPI_REAL4,IROOT-1,LOCALCOMM,KREQUEST_LOCAL,IERROR)
...
ELSE
  IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
    CALL MPI_GATHERV(PSENDBUF,ISENDCOUNT,MPI_REAL4,ZDUM_JPRM,ONES, &
     &  ONES,MPI_REAL4,IROOT-1,LOCALCOMM,IERROR)
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_IGATHERV(PSENDBUF,ISENDCOUNT,MPI_REAL4,ZDUM_JPRM,ONES, &
     &  ONES,MPI_REAL4,IROOT-1,LOCALCOMM,KREQUEST_LOCAL,IERROR)
    KREQUEST=KREQUEST_LOCAL%MPI_VAL
  ENDIF
  IF(LMPLSTATS) THEN
    CALL MPL_SENDSTATS(ISENDCOUNT,MPI_REAL4%MPI_VAL)
...
IF(PRESENT(KERROR)) THEN
  KERROR=IERROR
ELSE
  IF(IERROR /= 0 ) CALL MPL_MESSAGE(CDMESSAGE='MPL_GATHERV',&
...
IF(IPL_MYRANK == IROOT) THEN
  ...
  IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
    CALL MPI_GATHERV(PSENDBUF,ISENDCOUNT,MPI_REAL8,PRECVBUF,KRECVCOUNTS,&
     &  IRECVDISPL,MPI_REAL8,IROOT-1,ICOMM,IERROR)
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_IGATHERV(PSENDBUF,ISENDCOUNT,MPI_REAL8,PRECVBUF,KRECVCOUNTS,&
     &  IRECVDISPL,MPI_REAL8,IROOT-1,ICOMM,KREQUEST_LOCAL,IERROR)
    KREQUEST=KREQUEST_LOCAL%MPI_VAL
  ENDIF
  IF(LMPLSTATS) THEN
...
ELSE
  ...
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_IGATHERV(PSENDBUF,ISENDCOUNT,MPI_REAL8,ZDUM_JPRD,ONES, &
     &  ONES,MPI_REAL8,IROOT-1,ICOMM,KREQUEST_LOCAL,IERROR)
    KREQUEST=KREQUEST_LOCAL%MPI_VAL
  ENDIF
  IF(LMPLSTATS) THEN
    CALL MPL_SENDSTATS(ISENDCOUNT,MPI_REAL8%MPI_VAL)
...
IF(PRESENT(KERROR)) THEN
  KERROR=IERROR
ELSE
  IF(IERROR /= 0 ) CALL MPL_MESSAGE(CDMESSAGE='MPL_GATHERV',CDSTRING=CDSTRING,&
   & LDABORT=LLABORT)
ENDIF
...
IF(IPL_MYRANK == IROOT) THEN
  ...
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_IGATHERV(KSENDBUF,ISENDCOUNT,MPI_INTEGER,KRECVBUF,&
     & KRECVCOUNTS,IRECVDISPL,MPI_INTEGER,IROOT-1,ICOMM,KREQUEST_LOCAL,IERROR)
    KREQUEST=KREQUEST_LOCAL%MPI_VAL
  ENDIF
  IF(LMPLSTATS) THEN
...
ELSE
  IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
    CALL MPI_GATHERV(KSENDBUF,ISENDCOUNT,MPI_INTEGER,ZDUM_INT,ONES, &
     &  ONES,MPI_INTEGER,IROOT-1,ICOMM,IERROR)
  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
    CALL MPI_IGATHERV(KSENDBUF,ISENDCOUNT,MPI_INTEGER,ZDUM_INT,ONES, &
     &  ONES,MPI_INTEGER,IROOT-1,ICOMM,KREQUEST_LOCAL,IERROR)
    KREQUEST=KREQUEST_LOCAL%MPI_VAL
  ENDIF
  IF(LMPLSTATS) THEN
    CALL MPL_SENDSTATS(ISENDCOUNT,MPI_INTEGER%MPI_VAL)
...
IF(IPL_MYRANK == IROOT) THEN
  ...
  IF(PRESENT(KRECVDISPL).OR.PRESENT(KSENDCOUNT)) THEN
    ...
    ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
      CALL MPI_IGATHERV(KSENDBUF,ISENDCOUNT,MPI_INTEGER,KRECVBUF,&
       & IRECVCOUNTS,IRECVDISPL,MPI_INTEGER,IROOT-1,ICOMM,KREQUEST_LOCAL,IERROR)
      KREQUEST=KREQUEST_LOCAL%MPI_VAL
    ENDIF
    IF(LMPLSTATS) THEN
      CALL MPL_SENDSTATS(ISENDCOUNT,MPI_INTEGER%MPI_VAL)
  ...
  ELSE
    ...
    ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
      CALL MPI_IGATHER(KSENDBUF,ISENDCOUNT,MPI_INTEGER,KRECVBUF,&
       & ISENDCOUNT,MPI_INTEGER,IROOT-1,ICOMM,KREQUEST_LOCAL,IERROR)
      KREQUEST=KREQUEST_LOCAL%MPI_VAL
    ENDIF
    IF(LMPLSTATS) THEN
      CALL MPL_SENDSTATS(ISENDCOUNT,MPI_INTEGER%MPI_VAL)
  ...
  ELSE
    ...
    ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
      CALL MPI_IGATHER(KSENDBUF,ISENDCOUNT,MPI_INTEGER,ZDUM_INT,&
       & 1,MPI_INTEGER,IROOT-1,ICOMM,KREQUEST_LOCAL,IERROR)
      KREQUEST=KREQUEST_LOCAL%MPI_VAL
    ENDIF
    IF(LMPLSTATS) THEN
      CALL MPL_SENDSTATS(ISENDCOUNT,MPI_INTEGER%MPI_VAL)
...
IF(IPL_MYRANK == IROOT) THEN
  ...
  IF(PRESENT(KRECVDISPL)) THEN
    ...
    ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
      CALL MPI_IGATHERV(PSENDBUF,ISENDCOUNT,MPI_REAL8,PRECVBUF,&
       & IRECVCOUNTS,IRECVDISPL,MPI_REAL8,IROOT-1,ICOMM,KREQUEST_LOCAL,IERROR)
      KREQUEST=KREQUEST_LOCAL%MPI_VAL
    ENDIF
    IF(LMPLSTATS) THEN
      CALL MPL_SENDSTATS(ISENDCOUNT,MPI_REAL8%MPI_VAL)
  ...
  ELSE
    ...
    ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
      CALL MPI_IGATHER(PSENDBUF,ISENDCOUNT,MPI_REAL8,PRECVBUF(1),&
       & ISENDCOUNT,MPI_REAL8,IROOT-1,ICOMM,KREQUEST_LOCAL,IERROR)
      KREQUEST=KREQUEST_LOCAL%MPI_VAL
    ENDIF
    IF(LMPLSTATS) THEN
      CALL MPL_SENDSTATS(ISENDCOUNT,MPI_REAL8%MPI_VAL)
  ...
  ELSE
    ...
    ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
      CALL MPI_IGATHER(PSENDBUF,ISENDCOUNT,MPI_REAL8,ZDUM_JPRD,&
       & 1,MPI_REAL8,IROOT-1,ICOMM,KREQUEST_LOCAL,IERROR)
      KREQUEST=KREQUEST_LOCAL%MPI_VAL
    ENDIF
   IF(LMPLSTATS) THEN
      CALL MPL_SENDSTATS(ISENDCOUNT,MPI_REAL8%MPI_VAL)
