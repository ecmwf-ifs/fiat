...
SUBROUTINE MPL_GATHERV_PREAMB2(IPL_NUMPROC,IPL_MYRANK,IRECVBUFSIZE,ISENDCOUNT,&
     & KRECVCOUNTS,KIRECVDISPL,KIRECVDISPL_PT,KRECVDISPL,KREQUEST,CDSTRING)
...
LOGICAL PRESENT_RECVBUF

IDATA_TYPE=INT(MPI_REAL4)
PRESENT_RECVBUF=PRESENT(PRECVBUF)

#include "mpl_gather_array_tmpl.i90"
...
!!$       & KRECVCOUNTS,IRECVDISPL,IRECVDISPL_PT,KRECVDISPL,CDSTRING)
...
!!$     &  IRECVDISPL_PT,INT(MPI_REAL4),IROOT-1,ICOMM,IERROR)
...
!!$     &  IRECVDISPL_PT,INT(MPI_REAL4),IROOT-1,ICOMM,KREQUEST,IERROR)
!!$  ENDIF
!!$  IF(LMPLSTATS) THEN
...
!!$     &  IRECVDISPL_PT,INT(MPI_REAL8),IROOT-1,ICOMM,IERROR)
!!$  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
!!$    CALL MPI_IGATHERV(PSENDBUF(1),ISENDCOUNT,INT(MPI_REAL8),PRECVBUF(1),KRECVCOUNTS,&
!!$     &  IRECVDISPL_PT,INT(MPI_REAL8),IROOT-1,ICOMM,KREQUEST,IERROR)
!!$  ENDIF
!!$  IF(LMPLSTATS) THEN
...
PRESENT_RECVBUF=PRESENT(KRECVBUF)
...
!!$     & KRECVCOUNTS,IRECVDISPL_PT,INT(MPI_INTEGER),IROOT-1,ICOMM,IERROR)
!!$  ELSEIF(IMP_TYPE == JP_NON_BLOCKING_STANDARD .OR. IMP_TYPE == JP_NON_BLOCKING_BUFFERED) THEN
!!$    CALL MPI_IGATHERV(KSENDBUF(1),ISENDCOUNT,INT(MPI_INTEGER),KRECVBUF(1),&
!!$     & KRECVCOUNTS,IRECVDISPL_PT,INT(MPI_INTEGER),IROOT-1,ICOMM,KREQUEST,IERROR)
!!$  ENDIF
!!$  IF(LMPLSTATS) THEN
...
#endif
  CALL MPL_GATHERV_PREAMB2(IPL_NUMPROC,IPL_MYRANK,IRECVBUFSIZE,ISENDCOUNT,&
       & KRECVCOUNTS,IRECVDISPL,IRECVDISPL_PT,KRECVDISPL,KREQUEST,CDSTRING)
...
IF(IPL_MYRANK == IROOT) THEN
  ...
  IF(PRESENT(KRECVDISPL).OR.PRESENT(KSENDCOUNT)) THEN
    ...
    ENDIF
    CALL MPL_GATHERV_PREAMB2(IPL_NUMPROC,IPL_MYRANK,IRECVBUFSIZE,ISENDCOUNT,&
         & IRECVCOUNTS,IRECVDISPL,IRECVDISPL_PT,KRECVDISPL,KREQUEST,CDSTRING)
    IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
