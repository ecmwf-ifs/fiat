...
!     ------------------------------------------------------------------
     ...
     &        SUM,MAXVAL,setup_distvec,scatter_vector,gather_vector
      ...
      INTEGER ndv_chunk_size
      INTEGER nproc,myproc,mintet,mrealt,MT_distributed_vector
      INTEGER ,ALLOCATABLE :: nprcids(:)

      TYPE distributed_vector
        REAL, POINTER :: local(:)
        INTEGER       :: global_length,local_start,local_end,
     &   nchnks
      END TYPE distributed_vector

      INTERFACE ASSIGNMENT (=)
        MODULE PROCEDURE assign_ar_dv, assign_dv_ar,
     &                   assign_dv_dv, assign_scalar_dv
      END INTERFACE
     ...
     &                   power_dv_scalar
      ...
      INTERFACE DOT_PRODUCT
        MODULE PROCEDURE dot_product_dv_dv, dot_product_dv_ar,
     &                   dot_product_ar_dv
      END INTERFACE
...
! module procedures:
     ...
     &   kdv_chunk_size,kprcids)
        INTEGER kproc,kyproc,kintet,krealt,ktag,kdv_chunk_size 
        INTEGER kprcids(kproc)
        ...
        INTEGER FUNCTION nchunks (klen)
          INTEGER, INTENT(IN) :: klen
          nchunks = (klen+ndv_chunk_size-1)/ndv_chunk_size
          RETURN
        END FUNCTION nchunks

        INTEGER FUNCTION jend (kproc,klen)
          INTEGER, INTENT(IN) :: kproc,klen
          jend = MIN(  ndv_chunk_size
     &                *NINT( FLOAT(kproc)
     &                      *(FLOAT(nchunks(klen))/FLOAT(NPROC)))
     &               ,klen)
          RETURN
        END FUNCTION jend

        INTEGER FUNCTION jstart (kproc,klen)
          INTEGER, INTENT(IN) :: kproc,klen
...
!         copy array to the distributed_vector
     ...
     &     call ABOR1 ('error: store to unallocated distributed_vector')
          ...
          if (SIZE(pvec).EQ.SIZE(handle%local)) then
            handle%local(:) = pvec(:)
          else if (SIZE(pvec).EQ.handle%global_length) then
...
!         multiply array and distributed_vector
          ...
          REAL, DIMENSION(handle%local_start:handle%local_end) ::
     &      multiply_ar_dv
...
!         copy one distributed_vector to another
     ...
     &     call ABOR1('error: copy from unallocated distributed_vector')
          ...
          if (handle1%global_length.EQ.handle2%global_length) then
...
!         multiply two distributed_vectors
          ...
          REAL, DIMENSION(handle1%local_start:handle1%local_end) ::
     &      multiply_dv_dv
...
!         copy a section of a distributed_vector to array
          ...
          TYPE (distributed_vector), INTENT(IN)    :: handle
          REAL, DIMENSION(kstart:kend)             :: dvsection

          INTEGER itag,jroc,istart,iend,ierr,imsglen,
     &            isendr,itagr
...
!         collect the local parts from the other PEs
     ...
     &                         isendr,itagr,ierr)
                IF (ierr.LT.0)
     &            CALL ABOR1('DISTRIBUTED VECTOR ERROR IN MPE_RECV')
              ENDIF
            ENDIF
          ENDDO
...
!         synchronize
          ...
          IF (ierr.LT.0) CALL ABOR1('DOT_PRODUCT: ERROR IN MPE_BARRIER')
...
!         subtract distributed_vector and array
     ...
     &        handle%local(:)-pvec(handle%local_start:handle%local_end)
        ...
        REAL FUNCTION dot_product_helper (pvec1,pvec2,mystart,myend,
     &                                    klen,nchnks)
          REAL,    INTENT(IN), DIMENSION(:) :: pvec1,pvec2
          INTEGER, INTENT(IN)  :: mystart,myend,klen,nchnks

          INTEGER j,istart,iend,itag,jroc,ierr,imsglen,isendr,itagr,
     &            ichnk1,ichnk2,i2,iprocs,iroc,jj
          REAL, DIMENSION(nchnks) :: partial,zbuf
...
!   synchronize
        ...
        REAL FUNCTION dot_product_dv_dv (handle1,handle2)
     ...
     &       ('error: dot_product with unallocated distributed_vector')
          ...
          IF (handle1%global_length .ne. handle2%global_length)
     &      call ABOR1
     ...
     &                          handle1%global_length,handle1%nchnks)
        ...
        REAL FUNCTION dot_product_ar_dv (pvec,handle2)
          REAL,                      INTENT(IN) :: pvec(:)
          TYPE (distributed_vector), INTENT(IN) :: handle2
     ...
     &       ('error: dot_product different length distributed_vectors')
        ...
        REAL FUNCTION dot_product_dv_ar (handle1,pvec)
          TYPE (distributed_vector), INTENT(IN) :: handle1
          REAL,                      INTENT(IN) :: pvec(:)
     ...
     &       ('error: dot_product with unallocated distributed_vector')
          ...
          if (SIZE(handle1%local) .EQ. SIZE(pvec)) then
            dot_product_dv_ar =
     &        dot_product_helper (handle1%local,pvec,
     &                            handle1%local_start,handle1%local_end,
     &                            handle1%global_length,handle1%nchnks)
          else if (handle1%global_length .EQ. SIZE(pvec)) then
            dot_product_dv_ar =
     ...
     &       ('error: dot_product different length distributed_vectors')
        ...
        REAL FUNCTION sum_helper (pvec,mystart,myend,klen,nchnks)
          REAL,    INTENT(IN), DIMENSION(:) :: pvec
          INTEGER, INTENT(IN)  :: mystart,myend,klen,nchnks
...
!   synchronize
          CALL MPE_BARRIER (ierr)
          IF (ierr.LT.0) CALL ABOR1('SUM: ERROR IN MPE_BARRIER')

          RETURN
        END FUNCTION sum_helper

        REAL FUNCTION sum_dv (handle)
          TYPE (distributed_vector), INTENT(IN) :: handle
     ...
     &                  handle%global_length,handle%nchnks)
        ...
        REAL FUNCTION maxval_helper (pvec,mystart,myend,klen,nchnks)
          REAL,    INTENT(IN), DIMENSION(:) :: pvec
          INTEGER, INTENT(IN)  :: mystart,myend,klen,nchnks
...
!   synchronize
          CALL MPE_BARRIER (ierr)
          IF (ierr.LT.0) CALL ABOR1('MAXVAL: ERROR IN MPE_BARRIER')

          RETURN
        END FUNCTION maxval_helper

        REAL FUNCTION maxval_dv (handle)
          TYPE (distributed_vector), INTENT(IN) :: handle
...
!         distribute vector
     ...
     &     ('scatter_vector: copy to unallocated distributed_vector')
          ...
          if(PRESENT(pvec)) then
            if (SIZE(pvec).NE.handle%global_length) 
     &       call ABOR1
...
!         distribute vector
          ...
          REAL, INTENT(OUT)             :: parray(:)
          REAL,OPTIONAL, INTENT(IN)     :: pvec(:)
          INTEGER itag,jroc,ilen,ierr,imsglen,
     &            isendr,itagr
...
!         broadcast the array to the other PEs
            ...
            DO jroc=1,NPROC
              IF (jroc.NE.MYPROC) THEN
                CALL MPE_SEND (pvec,ilen,MREALT,
     &           NPRCIDS(jroc),itag,0,0,0,ierr)
                IF (ierr.LT.0)
     &           CALL ABOR1('DISTRIBUTED VECTOR ERROR IN MPE_SEND')
              ENDIF
...
!         collect the local parts from the distributing PE
            ...
            iend   = jend   (myproc,handle%global_length)
            IF (iend.GE.istart) THEN
              CALL MPE_RECV (iroc,1,MINTET,
     &         -1,itag,0,0,0,imsglen,isendr,itagr,ierr)
              IF (ierr.LT.0)
     &         CALL ABOR1('GATHER_VECTOR ERROR IN MPE_RECV')
              IF(NPRCIDS(iroc).ne.isendr)
     &         CALL ABOR1('GATHER_VECTOR : NPRCIDS(iroc).ne.isendr')
              CALL MPE_SEND (handle%local(:),iend+1-istart,MREALT,
