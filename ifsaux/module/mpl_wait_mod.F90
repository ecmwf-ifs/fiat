...
IF(PRESENT(KOUNT))THEN
  CALL MPI_GET_COUNT(IWAIT_STATUS(1),INT(MPI_REAL4),KOUNT,ICOUNTERR)
ENDIF
...
INTEGER(KIND=JPIM),INTENT(OUT),OPTIONAL :: KOUNT
INTEGER(KIND=JPIM),INTENT(OUT),OPTIONAL :: KERROR
INTEGER(KIND=JPIM) :: IWAITERR,ICOUNTERR,JL,ICOUNT
INTEGER(KIND=JPIM) :: IWAIT_STATUS(MPI_STATUS_SIZE)
LOGICAL :: LLABORT
LLABORT=.TRUE.
IWAITERR=0
...
write(0,*) 'IN MPL_WAIT1_REAL8 '
CALL MPI_WAIT(KREQUEST,IWAIT_STATUS,IWAITERR)

IF(PRESENT(KOUNT))THEN
  CALL MPI_GET_COUNT(IWAIT_STATUS(1),INT(MPI_REAL8),KOUNT,ICOUNTERR)
ENDIF
...
IF(PRESENT(KOUNT))THEN
  CALL MPI_GET_COUNT(IWAIT_STATUS(1),INT(MPI_INTEGER),KOUNT,ICOUNTERR)
ENDIF
...
IF(PRESENT(KOUNT))THEN
  CALL MPI_GET_COUNT(IWAIT_STATUS(1),INT(MPI_INTEGER8),KOUNT,ICOUNTERR)
ENDIF
