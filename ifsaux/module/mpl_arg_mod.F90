...
IF (N_ARGS == -1) THEN
  ...
  CALL MPI_COMM_SIZE(MPI_COMM_WORLD,INUMPROC,IERROR)
  ...
  IF (INUMPROC > 1) THEN
    ...
    CALL MPI_BCAST(IARGS,1,MPI_INTEGER,IROOT,MPI_COMM_WORLD,IERROR)
    ICOUNT = JP_ARGLEN
    IF (IRANK > 1) ALLOCATE(CL_ARGS(0:IARGS))
    IF (IRANK > 1) CALL PUTARG_INFO(IARGS, TRIM(CL_TERMINATE))
    DO J=0,IARGS
     ! The following broadcast does not use "mailbox" nor attached buffer, both potentially yet to be allocated
      CALL MPI_BCAST(CL_ARGS(J),ICOUNT,MPI_BYTE,IROOT,MPI_COMM_WORLD,IERROR)
