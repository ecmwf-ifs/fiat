...
REAL(KIND=JPRD),ALLOCATABLE :: ZSENDBUF(:)
...
IF(IMP_TYPE == JP_BLOCKING_STANDARD .OR. IMP_TYPE == JP_BLOCKING_BUFFERED) THEN
  IF (ISENDCOUNT == 0_JPIM) THEN 
    ALLOCATE(ZSENDBUF(1))
  ELSE
    ALLOCATE(ZSENDBUF(ISENDCOUNT))
    ZSENDBUF(:) = PSENDBUF(:)
  ENDIF  
  CALL MPI_ALLGATHERV(ZSENDBUF(1),ISENDCOUNT,INT(MPI_REAL8),PRECVBUF(1),&
