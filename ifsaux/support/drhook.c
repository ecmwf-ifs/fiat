...
process_options()
{
  ...
  if (env) {
    ...
    fprintf(stderr,">>>process_options(): DR_HOOK_CALLPATH_INDENT=%d\n",callpath_indent);
  }
  ...
  if (env) {
    ...
    fprintf(stderr,">>>process_options(): DR_HOOK_CALLPATH_DEPTH=%d\n",callpath_depth);
  }
  ...
  if (env) {
	...
	opt_callpath = 1;
	fprintf(stderr,"%s%s",comma,"CALLPATH"); comma = ",";
      }
      p = strtok(NULL,delim);
    }
    free_drhook(s);
