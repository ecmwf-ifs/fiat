...
!     ----------   MPE_BARRIER - syncronization of processors
...
IF(LSTATS) THEN
  IF(KNUM/=0) THEN
    IF(LSYNCSTATS .AND. NPROC>1.AND.(KSWITCH==0.OR. KSWITCH==2)) THEN
  ...
  IF(.NOT.((KSWITCH == 0.AND. IMOD == 1) .OR.&
   ...
   &(KSWITCH == 1.AND. IMOD == 0))) THEN
    WRITE(NULERR,*) 'KNUM,KSWITCH,IMOD,NCALLS(KNUM)',&
