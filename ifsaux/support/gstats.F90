...
IF(LSTATS) THEN
  ...
  IF(LGSTATS_LABEL)THEN
    ...
    DO INUM=100,199
      WRITE(CC,'(I4)')INUM
      CCDESC_DRHOOK(INUM)='>TRS-'//CCDESC(INUM)(1:21)//'('//CC//')'
    ENDDO
    DO INUM=250,300
      WRITE(CC,'(I4)')INUM
      CCDESC_DRHOOK(INUM)='>MPL-'//CCDESC(INUM)(1:21)//'('//CC//')'
    ENDDO
    DO INUM=500,1000
      WRITE(CC,'(I4)')INUM
      CCDESC_DRHOOK(INUM)='>MPL-'//CCDESC(INUM)(1:21)//'('//CC//')'
    ...
    DO INUM=1001,1700
      WRITE(CC,'(I4)')INUM
      CCDESC_DRHOOK(INUM)='>OMP-'//CCDESC(INUM)(1:21)//'('//CC//')'
    ENDDO
    LGSTATS_LABEL=.FALSE.
  ENDIF
!     ------------------------------------------------------------------
  ...
  IF(KNUM/=0) THEN
    IF(LSYNCSTATS .AND.(KSWITCH==0.OR. KSWITCH==2)) THEN
      IF(.NOT.OML_IN_PARALLEL().AND. KNUM < 500 )THEN
        IF(LHOOK)CALL DR_HOOK(CCDESC_BARR(KNUM),0,ZHOOK_HANDLE_BARR)
        CALL MPL_BARRIER(CDSTRING='GSTATS:')
        IF(LHOOK)CALL DR_HOOK(CCDESC_BARR(KNUM),1,ZHOOK_HANDLE_BARR)
      ENDIF
    ENDIF
  ENDIF

  IF (LHOOK .AND. (KSWITCH == 0 .OR. KSWITCH == 1)) THEN
    IF (KNUM > 100 .AND.KNUM < 199) THEN ! TRANS
      CALL DR_HOOK(CCDESC_DRHOOK(KNUM),KSWITCH,ZHOOK_HANDLE_TRANS)
    ELSE IF (KNUM > 250 .AND.KNUM < 300) THEN ! COMMS
      CALL DR_HOOK(CCDESC_DRHOOK(KNUM),KSWITCH,ZHOOK_HANDLE_COMMS)
    ELSE IF (KNUM > 1000 .AND.KNUM < 1700) THEN ! PARALLEL REGIONS
      CALL DR_HOOK(CCDESC_DRHOOK(KNUM),KSWITCH,ZHOOK_HANDLE)
    ELSE IF (KNUM > 500 .AND.KNUM < 1000.AND.KNUM.NE.682) THEN ! COMMS
