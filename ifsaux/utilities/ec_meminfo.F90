SUBROUTINE EC_MEMINFO(IU,IDSTRING,KCOMM)
...
!    CALL LINUX_TRBK()
  ...
  IF (.not.LLNOCOMM) THEN
     ...
     WRITE(KULOUT,'(a,i0,a,i0,a,i0,a,a,":",a,":",a,a,a,"-",a,"-",a)') &
          ...
          " computational & ",IOTASKS," I/O-tasks & ", OMP_GET_MAX_THREADS(), " threads at ", &
...
IF (NPROC > 1 .and. LLFIRST_TIME) THEN
   IORANK = 0
   IF (LEN(IDSTRING) >= 2) THEN
      IF (IDSTRING(1:2) == 'io' .or. IDSTRING(1:2) == 'IO') IORANK = 1
   ENDIF
   IF (MYPROC == 0) THEN
...
CALL EC_MEMINFO(KOUT,TRIM(CLSTEP),ICOMM)
CALL FLUSH(KOUT)
RETURN ! For now
CALL EC_GETHOSTNAME(NODENAME) ! from support/env.c
