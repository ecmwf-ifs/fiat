...
INTEGER(KIND=JPIB), SAVE :: ENERGY_START
...
IF (PAGESIZE == 0) THEN ! First time
   CALL EC_PMON(ENERGY_START,POWER)
...
IF (NPROC > 1 .and. LLFIRST_TIME) THEN
   ...
   IF (MYPROC == 0) THEN
      ...
      DO II=1,NPROC-1
         K = IDX(II)
         I = RN(K)%RANK
         CALL MPI_SEND(RN(K)%NODEMASTER,1,MPI_INTEGER4,I,ITAG+2,KCOMM,ERROR)
   ...
   ELSE
      ...
      CALL MPI_RECV(IAM_NODEMASTER,1,MPI_INTEGER4,0,ITAG+2,KCOMM,IRECV_STATUS,ERROR)
      CALL CHECK_ERROR("from MPI_RECV(IAM_NODEMASTER)",__FILE__,__LINE__)
   ENDIF
   LLFIRST_TIME = .FALSE.
   CALL CONDBARR()
   CALL CHECK_ERROR("from MPI_BARRIER(LLFIRST_TIME=.FALSE.)",__FILE__,__LINE__)
ENDIF
...
IF (MYPROC == 0) THEN
    ...
    DO II=1,NPROC-1
       ...
       ELSE
          NRECV = 4
       ENDIF
       CALL MPI_RECV(RECVBUF,NRECV,MPI_INTEGER8,I,ITAG+3,KCOMM,IRECV_STATUS,ERROR)
       CALL CHECK_ERROR("from MPI_RECV(RECVBUF)",__FILE__,__LINE__)
       NODENAME = RN(K)%NODE
       IF (LASTNODE==NODENAME) THEN
          ENERGY=MAX(ENERGY,RECVBUF(1))
          POWER=MAX(POWER,RECVBUF(2))
          HEAP_SIZE=HEAP_SIZE+RECVBUF(3)
          TASKSMALL=TASKSMALL+RECVBUF(4)
       ELSE
          PERCENT_USED(2) = 0
...
ELSE
    ...
    ELSE
       NSEND = 4
    ENDIF
    CALL MPI_SEND(SENDBUF,NSEND,MPI_INTEGER8,0,ITAG+3,KCOMM,ERROR)
...
ENDDO
NUMNODES = NODENUM
1000 FORMAT(/1X,5A12,1X,A12,1X,A12)
1001 FORMAT(1X,5I12,1X,A12,1X,I12)
1002 FORMAT(1X,3I12,A12,I12,1X,A12,1X,I12)
WRITE(6,1000) "#","NODE#","RANK#","I/O-TASK#","NODEMASTER","NODENAME","IDX"
DO I=0,NPROC-1
   IF (RN(I)%IORANK > 0) THEN
      WRITE(6,1001) I,RN(I)%NODENUM,RN(I)%RANK,RN(I)%IORANK,RN(I)%NODEMASTER,&
           & TRIM(ADJUSTL(RN(I)%NODE)),IDX(I)
   ELSE
      WRITE(6,1002) I,RN(I)%NODENUM,RN(I)%RANK,"(No)",RN(I)%NODEMASTER,&
           & TRIM(ADJUSTL(RN(I)%NODE)),IDX(I)
   ENDIF
ENDDO
WRITE(6,*)
