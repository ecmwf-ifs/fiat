...
IF (MYPROC == 0) THEN
    ...
    DO II=1,NPROC-1
        K = IDX(II)
        I = RN(K)%RANK
        CALL MPI_RECV(RECVBUF,SIZE(RECVBUF),MPI_INTEGER8,I,ITAG+3,KCOMM,IRECV_STATUS,ERROR)
        ...
        IF(LASTNODE==NODENAME)THEN
           ...
           HEAP_SIZE=HEAP_SIZE+RECVBUF(6)
           TASKSMALL=TASKSMALL+RECVBUF(7)
        ELSE
          PERCENT_USED(2) = 0
...
ELSE
    ...
    ENDDO
    CALL MPI_SEND(SENDBUF,SIZE(SENDBUF),MPI_INTEGER8,0,ITAG+3,KCOMM,ERROR)
